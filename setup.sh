...
git config --global init.templatedir "$HOOKS_DIRECTORY/gitpatrol_bootstrap"

# In turn, the bootstrap hooks will read this variable to figure out where to
# point the symlinks for the *real* hooks.
git config --global hooks.symlinksourcerepo "$HOOKS_DIRECTORY"

echo "...done with Git Patrol setup"
